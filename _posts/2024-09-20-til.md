...
로그에서 볼 수 있듯이 암호화 라이브러리인 `bcrypt` 쪽에서 발생한 에러였는데, 마침 node를 Alpine Linux 이미지로 사용하고 있는게 문제의 원인이었다.
...
- **Red Hat Linux**: Ubuntu Linux와 같이 기업이 관리하며 전세계적으로 엔터프라이즈 서버 환경 점유율이 가장 높다. 안정성을 우선한다.
...
- 도커 이미지를 Debian으로 바꿔서 해결할 수도 있다.
